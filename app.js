...
// -------------------------
const typeIcons = {
  fire:"🔥", water:"💧", grass:"🌿", ice:"🧊", flying:"🕊️", dark:"🌑", poison:"☣️", bug:"🐛", electric:"⚡"
};

const habitatIcons = {
  forest:"🌲", grassland:"🌾", mountain:"⛰️", freshwater:"🏞️", ocean:"🌊",
  urban:"🏙️", wetland:"🪷", desert:"🏜️", polar:"❄️", rainforest:"🌴"
};
...
// -------------------------
// ANIMALS DATA (simplified example, add your full list here)
// -------------------------
const animals = [
...
function updateBadges(){
  ...
  const createBadge = (text,title,extra="")=>{
  ...
  else if(continents.length>=3) createBadge("🌍 Explorer Badge",`Collected animals from 3+ continents.`,"badge-world");

  if(types.length>=6) createBadge("⚡ Element Master","Caught 6+ elements!","badge-element");
  else if(types.length>=3) createBadge("⚡ Elementalist Badge","Caught 3+ elements.","badge-element");

  if(logged.length>=10) createBadge("📘 Field Researcher","Logged 10+ animals!","badge-research");
  else if(logged.length>=5) createBadge("📖 Junior Researcher","Logged 5+ animals.","badge-research");

  if(rareCount>=5) createBadge("💎 Elite Collector","Collected 5+ rare animals!","badge-rare");
  else if(rareCount>=3) createBadge("💎 Collector Badge","Collected 3+ rare animals.","badge-rare");
    ...
    card.innerHTML = `
      ...
      <div class="card-body">
        ...
        <div class="footer">
          ...
          ${a.habitat?`<span>${habitatIcons[a.habitat]} ${a.habitat}</span>`:""}
...
// -------------------------
// WORLD MAP (simplified example)
// -------------------------
function updateWorldMap(){
  // implement map updates if you have continent shapes
}
